...
var _ = FDescribe("v3-set-droplet Command", func() {
	var (
	...
	Context("when the droplet has been set to the app", func() {
		BeforeEach(func() {
			...
			fakeActor.SetAppDropletReturns(v3action.Warnings{"warning-1", "warning-2"}, nil)
		})
		...
		It("displays that the droplet was assigned", func() {
			...
			Expect(fakeActor.SetAppDropletCallCount()).To(Equal(1))
			appName, dropletGUID := fakeActor.SetAppDropletArgsForCall(0)
	...
	Context("when the actor returns an error", func() {
		...
		BeforeEach(func() {
			...
			fakeActor.SetAppDropletReturns(v3action.Warnings{"warning-1", "warning-2"}, expectedErr)
		})
